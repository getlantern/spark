...
/// flow's upstream through `transport`.
///
...
pub async fn run<N: Netstack>(mut netstack: N, transport: Arc<dyn Transport>) {
    while let Some(flow) = netstack.accept_tcp().await {
        tokio::spawn(forward(flow, Arc::clone(&transport)));
    }
...
/// either side closes.
async fn forward(flow: TcpFlow, transport: Arc<dyn Transport>) {
    let TcpFlow {
        original_dst,
        src,
        mut stream,
    ...
    let mut upstream = match transport.dial(original_dst).await {
        Ok(s) => s,
        Err(e) => {
    ...
    // `&mut *stream` / `&mut *upstream` deref each box to `&mut (dyn AsyncReadWrite + Unpin
    // + Send)`, which satisfies `copy_bidirectional`'s `AsyncRead + AsyncWrite + Unpin +
    // ?Sized` bound.
...
mod tests {
    ...
    async fn assert_echo_through(transport: Arc<dyn Transport>, original_dst: SocketAddr) {
        ...
        tokio::spawn(run(OneFlow(Some(flow)), transport));
