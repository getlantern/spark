...
> **Status (2026-07-01):** M1 ✅ · M2 ✅ · M3 ✅ (wired at L3) · M4 ✅ (fake-IP DNS wired end-to-end, M4.1–M4.6c) · exit-side resolution for tunnel/shadowsocks/hysteria2 ✅ · M5 🟢 (footprint measured: android release `.so` 7.67 MB, smart-routing adds ~100 KB; **parse-perf O(n²)→O(n log n) fixed**, banad 2.43 s→0.01 s; on-device RAM/jank pending the Redmi) · M6 ✅ (cache/refresh/offline + kindling fetch + `fd_tunnel` wiring) · **activated** in the Android + Apple builds (verified: aarch64-android cross-build via cargo-ndk). All committed on `fisk/smart-routing-design`; every step green (workspace builds + `clippy -D warnings` + `fmt` on no-feature / `smart-routing` / `config-fetch` / `all-features`). The full feature is code-complete AND compiled into the mobile builds: DNS interception → fake IP → per-flow Direct/Proxy/Reject with exit-side resolution → auto-updating rule-sets via kindling. **Remaining** (all on-device/network, tracked in `docs/smart-routing-on-device-validation.md`): (1) on-device RAM/jank + parse timing on the Redmi (hardware pending); (2) live validation of the kindling `.srs` fetch (host the rule-sets on a frontable CDN). PR #42 reviewed (6 Copilot rounds) + an end-to-end composition test added; ready to merge.
...
### M5 — mobile footprint  🟢 (footprint + parse-perf done; on-device RAM/jank pending hardware)
...
- **Footprint measured (2026-07-01):** the release `aarch64-linux-android` `libspark_android.so` (fat LTO, `opt-level=z`, stripped) is **7.67 MB** with `smart-routing` vs **7.57 MB** without → **smart-routing adds ~0.10 MB (~100 KB)**. The 7.57 MB base is BoringSSL + QUIC (hysteria2) + the 6 transports + config-fetch/kindling — the legacy <3 MB target predates all of that; smart-routing's own contribution is negligible. (Release `spark` CLI is 2.16 MB.) On-device RAM/jank still needs the physical Redmi.
- **⚠ Parse-perf finding — FIXED (2026-07-01):** `srs::parse(banad_v1)` was **~2.43 s release / ~40 s debug**. Root cause: `SuccinctSet::keys()` called `count_zeros`/`select_ith_one` (linear bitmap scans) once per trie edge → **O(n²)**. Replaced with a `BitRank` index (cumulative per-word popcounts): O(1) rank, O(log words) select → keys() ~O(n log n). **banad parse now ~0.01 s release**; the full smart-routing lib suite dropped ~68 s → ~6 s. Correctness preserved (v3 oracle exact-match + spot-checks unchanged; added a BitRank unit test). So startup parse cost is no longer a concern; on-device profiling just needs to confirm on the Redmi's slower CPU.

### M6 — ruleset fetch / cache / refresh + offline ✅
- **Deliverable:** `rules/ruleset.rs` — fetch the `.srs` URLs, disk-cache under `data_dir`, refresh in the background, mirroring config-fetch; offline uses cache; missing cache degrades to proxy-everything.
- **M6a (core):** `cache_path` (matches the tunnel loader), a `RuleSetFetcher` seam, `refresh_one`/`refresh_all` (**validate-before-cache**: a fetch failure or a non-parsing download never disturbs the existing cache — offline-resilient), atomic `write_atomic`, `run_refresh_loop`. Hermetic fake-fetcher tests.
- **M6b (kindling fetch + wiring):** `KindlingRuleSetFetcher` fetches every `.srs` **exclusively via kindling** (`flint_kindling::FrontedBootstrap` — domain-fronted, self-bootstrapping; **no direct-dial fallback**, per product requirement), seeded per-device (`config::fetch::seed_from_device_id`). `run_refresh_loop` is now **staleness-aware** (mtime vs interval) so a warm cache isn't re-fetched every connect. Wired into `fd_tunnel::run_fd_lantern_api` next to the config refresh (12 h interval, `smart-routing`-gated); cache warms in the background, rules apply next connect (no live router swap in v1). **Caveat:** the `.srs` host must be **frontable** (served by a CDN kindling knows — CloudFront/Akamai/Aliyun); host the rule-sets accordingly.
- **Verify:** offline/corrupt/missing-cache behaviors unit-tested (M6a); URL parse + staleness unit-tested (M6b). Cold-fetch + live-refresh over kindling need on-device/live validation (the fetch itself isn't hermetically testable).
