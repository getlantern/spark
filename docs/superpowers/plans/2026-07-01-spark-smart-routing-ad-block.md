...
> **Status (2026-07-01):** M1 ✅ · M2 ✅ · M3 ✅ (wired at L3) · M4 ✅ (fake-IP DNS wired end-to-end, M4.1–M4.6c) · exit-side resolution for tunnel/shadowsocks/hysteria2 ✅ · M5 🟡 (host binary-size check done, 2.16 MB; on-device RAM/jank + parse-perf pending the Redmi) · M6 ✅ (cache/refresh/offline + kindling fetch + `fd_tunnel` wiring). All committed on `fisk/smart-routing-design`; every step green (workspace builds + `clippy -D warnings` + `fmt` on no-feature / `smart-routing` / `config-fetch` / `all-features`). The full feature is code-complete: DNS interception → fake IP → per-flow Direct/Proxy/Reject with exit-side resolution → auto-updating rule-sets via kindling. **Remaining:** (1) enable the `smart-routing` cargo feature in the mobile build (a build-config flip) to activate it; (2) on-device M5 profiling (RAM/jank + the ⚠ parse-perf finding on the Redmi); (3) live validation of the kindling `.srs` fetch (host the rule-sets on a frontable CDN).
