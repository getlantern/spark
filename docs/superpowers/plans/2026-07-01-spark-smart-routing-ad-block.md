...
- **M4.5 — `Address` dial-by-name** (transport layer). Add `pub enum Address { Ip(SocketAddr), Domain { host: String, port: u16 } }`; change `Transport::dial(&self, Address)`; update every impl (domain → carry to exit for proxy protocols; direct → resolve via local resolver then dial IP). Keep `SocketAddr`-callers working via `Address::from(SocketAddr)`. Verify: existing transport tests pass with `Address::Ip`; a domain target reaches a stub exit as a name.
