...
- **macOS (confirmed on-device 2026-07-10):** the NE (`org.getlantern.spark.tunnel`) runs as **root**, and `PacketTunnelProvider.swift` self-resolves its cache dir via `containerURL(forSecurityApplicationGroupIdentifier:)` — which resolves **per-user**, so as root it lands in `/var/root/Library/Group Containers/group.org.getlantern.spark/config/config_raw.json` (`root:wheel`, `644`). That's a *different physical dir* than the user app's `~/Library/Group Containers/...`, and `/var/root` is mode `700` so the user cannot even traverse into it. The app's Phase 1 read therefore finds nothing today.
  - **❌ REJECTED approach — "point the NE at the user's container via `providerConfiguration["dataDir"]`".** Tried and **broke connect** (confirmed on-device 2026-07-13). The **system-extension sandbox denies the root NE access to the *user's* Group Container**: `load_or_fetch` fails with `Operation not permitted (os error 1)`, the self-fetch never completes, and the tunnel hangs in "connecting" until it times out. The NE can only use its **own** (root's) container. There is **no shared filesystem path** between the root sysext and the user app via Group Containers — root's container is `/var/root/...` (mode `700`, user can't read) and the user's container is sandbox-forbidden to the NE. This approach was reverted.
  - **✅ Actual macOS design — no sharing; the app is self-sufficient.** The NE keeps caching into its **own** container (for the tunnel). The **app maintains its own separate copy** in *its* container (`~/Library/Group Containers/group.org.getlantern.spark/config`, which the non-sandboxed app can freely read/write) — populated by the **Phase 2a** app-side startup fetch and read back by the Phase 1 `servers()` fallback. Both processes fetch the same config-new upstream, so the two caches are equivalent; they simply aren't shared. Consequence: **Phase 1 alone shows nothing on macOS** (its read target is empty until something writes it) — Phase 1 + Phase 2a ship together, and 2a is what populates the app's cache. The whole "one canonical copy / file-locking" line of design is **moot on macOS** (nothing is shared cross-process).
