...
| **Android** | **WORKS as of 2026-08-03**, after fixing a real bug: the redirect listener was bound to the tun's address but not the tun *device*, so the kernel never delivered to it (sing-tun binds both). TCP now carries 53 Mb/s through the tunnel. **UDP loss is 13% vs userspace's 0.043% — open defect, blocks a default flip.** See `docs/android-system-stack-gate.md` §0c. Previously: **BROKEN — measured 2026-08-03.** Initializes cleanly (arm confirmed, no smoltcp, no panic) and forwards nothing: 100% packet loss through the tunnel while userspace on the same device/peer did 58–62 Mb/s. `rp_filter` is already 0, so the documented Linux precondition is not the cause. See `docs/android-system-stack-gate.md` §0a. Previously assessed as: **Viable** (2026-06-17) | Android's `VpnService` hands the app a **Linux tun fd**; sing-tun adopts it (`tun_linux.go New()`, `FileDescriptor != 0` branch) with no platform restriction, and **sing-box runs `stack: system` on Android** this way. spark's android build just doesn't *enable* the `system-stack` feature yet — a choice, not a limit. Android-specific plumbing: use `VpnService.protect()` for upstream-socket protection (vs. `IP_UNICAST_IF`). This means the collapse fix could reach Android, not only desktop. |
