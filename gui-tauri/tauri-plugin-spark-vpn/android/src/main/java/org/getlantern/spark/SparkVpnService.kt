...
class SparkVpnService : VpnService() {
    ...
    private fun startInForeground() {
        ...
        // Reopen the app when the notification is tapped. This lives in the plugin *library*, which
        // can't reference the app's MainActivity class directly, so resolve the launch intent at
        // runtime from the package manager (null if there's no launcher activity — then skip it).
        // FLAG_IMMUTABLE is required on API 23+.
